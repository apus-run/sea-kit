...
import (
	...
	collection "github.com/apus-run/sea-kit/collection/timingwheel"
	"github.com/apus-run/sea-kit/mathx"
	"github.com/apus-run/sea-kit/syncx"
)
...
type (
	...
	// A Cache object is an in-memory tiny_cache.
	Cache struct {
		name           string
		lock           sync.Mutex
		data           map[string]any
...
func (c *Cache) Take(key string, fetch func() (any, error)) (any, error) {
	...
	val, err := c.barrier.Do(key, func() (any, error) {
		...
		// so we do double check, tiny_cache might be taken by another call
		if val, ok := c.doGet(key); ok {
			return val, nil
		}
...
func (cs *cacheStat) statLoop() {
	...
	for range ticker.C {
		...
		log.Printf("tiny_cache(%s) - qpm: %d, hit_ratio: %.1f%%, elements: %d, hit: %d, miss: %d",
