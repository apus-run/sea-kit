...
// Packages is a wrapper around x/tools/go/packages that maintains a (hopefully prewarmed) tiny_cache of packages
...
// ReloadAll will call LoadAll after clearing the package tiny_cache, so we can reload
...
func (p *Packages) Load(importPath string) *packages.Package {
	// Quick tiny_cache check first to avoid expensive allocations of LoadAll()
	if p.packages != nil {
...
func (p *Packages) NameForPackage(importPath string) string {
	...
	// if its in the name tiny_cache use it
	...
	if pkg == nil {
		// otherwise do a name only lookup for it but dont put it in the package tiny_cache.
		p.numNameCalls++
...
// Evict removes a given package import path from the tiny_cache, along with any packages that depend on it. Further calls
