...
func TestErrorDiscoverer(t *testing.T) {
	...
	r := New(notifier, discoverer, 3, make(chan []string, 100), log.L(), true)
...
func TestGRPCResolverRoundRobin(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(fmt.Sprintf("%+v", test), func(t *testing.T) {
			res := New(notifier, discoverer, test.minPeers, make(chan []string, 100), log.L(), true)
...
func TestRendezvousHash(t *testing.T) {
	...
	notifier := &Dispatcher{}
	discoverer := FixedDiscoverer{}
	resolverInstance := New(notifier, discoverer, 2, make(chan []string, 100), log.L(), true)
